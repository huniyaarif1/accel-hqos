...
static void update_shaper_tr(struct shaper_pd_t *pd)
{
	struct time_range_pd_t *tr;
         ...
         //log_emerg(" ******** TIme Shift is Called ******* ");
...
out:
	...
	log_emerg(" ******** TIme Shift is Called 2 ******* ");

	char* tmp2=pd->ses->ifname;

	log_emerg(tmp2);
         //system("/etc/ppp/ip-up");
	
              char* tmp="/etc/ppp/ip-up ";
	      char*  str3 = (char* ) malloc(1 + strlen(tmp)+ strlen(tmp2) );
              strcpy(str3, tmp);
              strcat(str3, tmp2);
		
              const char* path=str3;
		//printf("update shaper path %s\n", path);
		//log_debug("shaper: interface%s\n", path);
